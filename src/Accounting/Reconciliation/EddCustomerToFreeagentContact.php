...
class EddCustomerToFreeagentContact {
	...
	protected function updateContactUsingPaymentInfo() {
		...
		$contact = ( new Update() )
			...
			->setFirstName( (string)$originalContact->first_name )
			->setLastName( (string)$originalContact->last_name )
			->setEmail( $p->email )
			...
			->setOrganisationName( (string)$p->get_meta( VAT::META_META_VAT_COMPANY ) )
